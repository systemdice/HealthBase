...
import { CompanyMaster, LabTestIndividual, NewModifyCase, NotifyPendingTestData } from '../models/UserData';
...
@Component({
  ...
  styleUrls: ['./company-master-details.component.css']
})
export class CompanyMasterDetailsComponent  implements OnInit {
  displayedColumns: string[] = ['slno', 'UnqueID', 'DateStart','CompanyName','CompanyID'];
  ...
  asyncPipeNewModifyCase: CompanyMaster[] = [];
  asyncPipeNewModifyCaseBackup: CompanyMaster[] = [];

  constructor(private companyMasterService: CompanyMasterDetailsService,private addModifyCaseService: AddModifyCaseService, private labTestIndividualService: LabTestIndividualService,
  ...
  ngOnInit() {
    ...
    this.companyMasterService.getAll().subscribe((data: CompanyMaster[]) => {
      //console.log('jay'+data);
