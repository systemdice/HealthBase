...
@Component({
  selector: 'app-case-farmacy-details-main',
  templateUrl: './case-farmacy-details-main.component.html',
  styleUrls: ['./case-farmacy-details-main.component.css']
})
export class CaseFarmacyDetailsMainComponent implements OnInit {
  displayedColumns: string[] = ['slno', 'UnqueID', 'DateStart','PatientName','DoctorName', 'Pharmacy','MedicineList',];
  ...
  ngOnInit() {
    ...
    this.addModifyCaseService.getFarmaCreditDetails(this.Username).subscribe((data: NewModifyCase[]) => {
      //console.log('jay'+data);
      ...
      else
      {
        ...
        this.asyncPipeNewModifyCase = data.filter(a => a.Home.AssignedPharma == this.Username);
        this.asyncPipeNewModifyCaseBackup = data.filter(a => a.Home.AssignedPharma == this.Username);//.filter(a => a.Location == this.Location);
      }
  ...
  addEditExpenseItem(expenseId: NewModifyCase, firstName: string,newIDParam:string) {
    if (expenseId !== null) {
      this._router.navigate(['/Farmaentry', expenseId.UnqueID], { queryParams: { username: firstName,newid:expenseId.IPDOPDId } });
    }
    else {
