...
import { LabTestIndividual, FarmacyDeliveryToPatient, NotifyPendingTestData, NewModifyCase } from '../models/UserData';
...
import { AllMasterFixedData, APIDetails } from 'src/app/shared/AllConstants';
import { Chart } from 'chart.js';
import { JsonpClientBackend } from '@angular/common/http';


@Component({
  selector: 'app-account-book-patient-wise',
  templateUrl: './account-book-patient-wise.component.html',
  styleUrls: ['./account-book-patient-wise.component.css']
})
export class AccountBookPatientWiseComponent implements OnInit {
  displayedColumns: string[] = ['slno', 'BillNo', 'PharmacyStoreName','BillingDate', 'IPDOPDId', 'CustomerName', 'CeditStatus', 'GrossSalePriceOnthisBill'];
  ...
  reportArryX: any[] = [];
  reportArryY: any[] = [];
  StoreNameCollection : string[] = AllMasterFixedData.storeNames;
...
this.StartDatePL= null;
  ...
  saveLabTest() {
    if (this.selectedStore == ''){
      alert('Please select a Pharmacy store.');
      return;
    } 
    this.total = 0;
    this.farmacyEntryService.getCashFarmDataNamewise(this.selectedStore).subscribe((data: FarmacyDeliveryToPatient[]) => {
      //console.log('jay'+data);
      ...
      for (var i = 0; i < this.asyncPipeFarmacyDeliveryToPatientBackup.length; i++) {  //loop through the array
        var varTotal = (this.asyncPipeFarmacyDeliveryToPatientBackup[i].GrossSalePriceOnthisBill == '' || this.asyncPipeFarmacyDeliveryToPatientBackup[i].GrossSalePriceOnthisBill==null) ? '0' : this.asyncPipeFarmacyDeliveryToPatientBackup[i].GrossSalePriceOnthisBill;
        this.total += +varTotal; // +this.asyncPipeNewModifyCase[i].PaymentHistory[0].EarlierPayment;  //Do the math!
      }
